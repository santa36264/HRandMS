...
{
    /**
     * Handle incoming Telegram webhook updates for admin bot
     */
    public function handle(Request $request): Response
    {
        try {
            ...
            Log::info('Telegram Admin Bot webhook received', [
            ...
            if (!$this->verifyWebhook($request)) {
                Log::warning('Invalid admin webhook request');
                return response()->json(['ok' => false], 403);
            }

            // Create and handle with bot
            $bot = BotFactory::create('admin');
            $bot->handle($update);

            return response()->json(['ok' => true]);
        } catch (\Exception $e) {
            Log::error('Telegram admin webhook error', [
                'error' => $e->getMessage(),
